...
module.exports = bot => {
  ...
  points.forEach((p, index) => {
    bot.action(`pollution${index}`, ({ replyWithPhoto, next }) => {
      ...
      }).then(() => replyWithPhoto({ source: tempFileName })).then(() => next()).catch(console.log);
    });
  });
  ...
  bot.command('pollution', function({ reply }) {
    ...
    return reply('Выберите место', Markup.inlineKeyboard(keyboard).extra())
  });
};
